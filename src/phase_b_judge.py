...
from config import LLM_API_KEY, LLM_BASE_URL, JUDGE_MODEL, HUMAN_LABELS_PATH, TEST_SET_PATH
...
if __name__ == "__main__":
    ...
    # Judge model_answer vs the test set's ground_truth: A=model_answer, B=ground_truth.
    # winner A or tie → judge thinks model_answer is correct (1); winner B → incorrect (0).
    with open(TEST_SET_PATH, encoding="utf-8") as f:
        test_set = json.load(f)
    ground_truths = {item["id"]: item["ground_truth"] for item in test_set}

    judge_results = []
    judge_labels = []
    for item in human_data:
        gt = ground_truths.get(item["question_id"], "")
        jr = swap_and_average(item["question"], item["model_answer"], gt)
        judge_results.append(jr)
        judge_labels.append(0 if jr.final_winner == "B" else 1)
    ...
    report = {
        ...
        "bias_report": bias,
        "demo_swap_and_average": {
            "question": result.question, "winner_pass1": result.winner_pass1,
            "winner_pass2": result.winner_pass2, "final_winner": result.final_winner,
