...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    # Restrict to entities relevant to this lab. The default PERSON/US_* recognizers
    # are tuned for English NER and misfire heavily on Vietnamese text (e.g. flagging
    # "nghỉ phép năm 2024" as a person's name), so they're excluded here.
...
if __name__ == "__main__":
    # Initialize engines once — reused across all calls below (avoids re-loading
    # spaCy/NeMo on every pii_scan/check_input_rail call, which would inflate latency).
    analyzer, anonymizer = setup_presidio()
    rails = setup_nemo_rails()
    ...
    report = {
        "pii_scan_demo": result,
        "adversarial_suite": results,
        "adversarial_pass_rate": (sum(1 for r in results if r["passed"]) / len(results)
                                   if results else 0.0),
        "latency": latency,
    }
