...
from config import TEST_SET_PATH, LLM_API_KEY, LLM_BASE_URL, LLM_MODEL, EMBEDDING_MODEL
...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    try:
        ...
        # DeepSeek (OpenAI-compatible) as judge LLM; local sentence-transformers for embeddings
        # since DeepSeek has no embeddings endpoint.
        llm = ChatOpenAI(model=LLM_MODEL, api_key=LLM_API_KEY, base_url=LLM_BASE_URL, temperature=0)
        embeddings = HuggingFaceEmbeddings(model_name=EMBEDDING_MODEL)

        # DeepSeek only supports n=1; answer_relevancy defaults to strictness=3 (n=3 sampling).
        answer_relevancy.strictness = 1

        # Low concurrency + retries: avoids OpenRouter's in-flight budget errors on tight credit.
